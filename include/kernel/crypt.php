...
class encrypt
{
	...
	function process($pass,$crypt_direction,$secure='',$key='')
	{
		if($key == '')
		{
			if($secure=='lic34543#$%^&^%')
			{
				$str = 'NUBYVCTRX5tyuioKPLKo9j8h7g6f5D$R%^GYHU*JIkolK(J*H&G6f5d4%FG^YHUNimNBYVCRXEEXRCT^YB&U98&^%C4x3innubyvtcrxecrtR';
			}
			else
			{
				$str = $key.'^sJksfyj%rhahrsh$tthjyjnd#fSRHt@jtjasr!hR';
			}
			for($i = 1, $total = strlen($str); $i < $total; $i = round(($i+1)*2))
			{
				$key .= md5($str{$i});
			}
		}
		...
		$new_pass = $cipher->$crypt_method($pass, $key);
		unset($cipher);
		return $new_pass;
	}
}
