...
class Files extends Controller 
{
	...
	public function get($id='', $name='', $error='')
	{
		...
		if((!$data['captcha_bool'] and $data['sec'] <= 1 and $data['auto_download']) or ($file->direct and $file->direct_bw > 0))
		{
		...
		// If the user is logged in, vut their wait time in 1/2
		if($this->session->userdata('id'))
		{
			$data['sec'] = round($data['sec'] / 2);
		}
	...
	public function gen($id='', $name='')
	{
		...
		if(!$this->input->post('captcha') and $this->startup->group_config->download_captcha)
		{
		...
		if($this->startup->group_config->download_captcha)
		{
