...
class CI_Loader {
	 ...
	 * Load Extention
	 *
	 * This function loads the specified extention.
	 *
	 * @access	public
	 * @param	array
	 * @return	void
	 */
	function extention($extentions = array())
	{
		...
		{	
			$plugin = strtolower(str_replace(EXT, '', $extention));		

			if (isset($this->_ci_extentions[$extention]))
			{
				continue;
			}

			if (file_exists(APPPATH.'extend/'.$extention.'/main'.EXT))
			{
				include_once(APPPATH.'extend/'.$extention.'/main'.EXT);	
			}
			else
			{
				if (file_exists(BASEPATH.'extend/'.$extention.'/main'.EXT))
				{
					include_once(BASEPATH.'extend/'.$extention.'/main'.EXT);	
				}
				else
				{
					show_error('Unable to load the requested file: extend/'.$extention.'/main'.EXT);
				}
			}
			
			$this->_ci_extentions[$extention] = new $plugin();
			log_message('debug', 'Extention loaded: '.$plugin);
		}		
	}
