...
<table border="0" style="width:95%" id="file_list_table">
	...
	<tr id="<?=$file->file_id?>" <?=alternator('class="odd"', 'class="even"')?>>
		...
		<td>
			...
			<a href='javascript:;' onclick='$("#<?=$file->file_id?>-edit").toggle()'><img src="<?=$base_url.'/img/icons/edit_16.png'?>"  title="<?php echo $this->lang->line('files_manage_17') ?>" class="nb" /></a>
	...
	<tr class="details" style="display:none; border-top:none;" id="<?=$file->file_id?>-edit">
		<td colspan="4" id="<?=$file->file_id?>-edit-inner">
			...
			<input name="<?=$file->id?>_feat" id="<?=$file->id?>_feat" <?php if($file->featured){?> checked="checked"<? }?> type="checkbox" value="1" /> <?php echo $this->lang->line('files_manage_14') ?><br /><br />
            
			<?=generateLinkButton($this->lang->line('files_manage_15'), 'javascript:;', base_url().'img/icons/ok_16.png', 'green', array('onclick' => 'editFileProps("'.$file->id.'");$("#'.$file->file_id.'-edit").hide();'))?><br /><br />
		</td>
	</tr>
...
<script type="text/javascript">
	function editFileProps(id)
	{
		...
		alert(fDesc+' :: '+fPass+' :: '+curFileId+' :: '+fFeatured);
		$.post(
