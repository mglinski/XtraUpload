...
<?php if($file->type == 'mp3' and $this->startup->site_config['show_preview'])
{
	?>
	<h3 id="dlhere"><img src="<?php echo $base_url?>img/icons/music_16.png" class="nb" alt="" /> <?php echo $this->lang->line('files_get_13')?></h3>
	<p>
		<span id="player"><?php echo $this->lang->line('files_get_14')?></span><br />
		<a href="javascript:;" onclick="$('#mp3_embed_code').slideToggle('normal')">
			...
			<input style="display:none" id="mp3_embed_code" type="text" size="60" onclick="this.select();" onfocus="this.select()" value='<iframe src="<?=site_url('files/embed/mp3/'.$file->file_id)?>" width="470" height="20" scrolling="no" frameborder="0"></iframe>' />
	</p>
...
<div>
	...
	<?php if($this->startup->group_config->download_captcha)
	{
		?>
